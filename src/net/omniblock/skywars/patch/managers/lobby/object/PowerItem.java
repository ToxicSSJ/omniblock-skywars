...
public interface PowerItem {
	...
	public static class PowerItemManager {
		...
		public static void applyVotes() {
			...
			if (TIME_ALTERATOR != PowerItemType.TIME_ALTERATOR_DEFAULT)
				actived_powers.add(TIME_ALTERATOR);
			setupPowerItems();
		...
		private static void setupPowerItems() {
			...
			if (actived_powers.size() <= 0)
				new TimeAlterator().setup();
			...
			for (PowerItemType pit : actived_powers) {
				...
				if (pit == PowerItemType.TIME_ALTERATOR_AFTERNOON || pit == PowerItemType.TIME_ALTERATOR_DEFAULT
						|| pit == PowerItemType.TIME_ALTERATOR_MIDDAY || pit == PowerItemType.TIME_ALTERATOR_MIDNIGHT
						|| pit == PowerItemType.TIME_ALTERATOR_MORNING || pit == PowerItemType.TIME_ALTERATOR_NIGHT) {
				...
				if (pit == PowerItemType.MORE_INSANE_ITEMS || pit == PowerItemType.MORE_LEGENDARY_ITEMS) {

					if(pit == PowerItemType.MORE_INSANE_ITEMS){
						Chests.FILLER.makeFill(ChestFillType.MORE_ITEM_IMPROVED_CHEST);
						continue;
					}
					
					Chests.FILLER.makeFill(ChestFillType.MORE_ITEM_Z_CHEST);
					continue;
				...
				if (pit == PowerItemType.CONTAMINATION) {
					...
					if (Skywars.currentMatchType == SkywarsType.SW_INSANE_SOLO) {
						...
						TeamSkywars.mainRunnableTask.addEvent("&d&lCONTAMINACIÓN:", 120);
					}
		...
		private static void broadcastModule(PowerItemType pit) {
			...
			if (pit == PowerItemType.TIME_ALTERATOR_AFTERNOON || pit == PowerItemType.TIME_ALTERATOR_DEFAULT
				...
				Bukkit.broadcastMessage(
						TextUtil.format("&9&lMODULO ACTIVADO: &7El tiempo ha sido definido a: &6" + pit.getName()));
				return;
	...
	public static enum PowerItemType {
		...
		TIME_ALTERATOR_MORNING("Madrugada", "elegir el tiempo de madrugada. "), TIME_ALTERATOR_MIDDAY("Medio dia",
				"elegir el tiempo de dia. "), TIME_ALTERATOR_AFTERNOON("Atardecer",
						"elegir el tiempo de atardecer. "), TIME_ALTERATOR_NIGHT("Noche",
								"elegir el tiempo de noche. "), TIME_ALTERATOR_MIDNIGHT("Media noche",
										"elegir el tiempo de media noche. "),

		CONTAMINATION("Contaminación", "activar la contaminación. "), DAYS_BLAZER("Dios de los dias",
				"activar al Dios de los dias. "),

		MORE_INSANE_ITEMS("Más Items Mejorados", "activar más items insanos. "), MORE_LEGENDARY_ITEMS(
				"Más Items Legendarios", "activar más items legendarios. "),
