...
public class ChestFillerHandler {
	...
	public enum ChestFillType {
		...
		COMMON_FILL_CHEST, COMMON_REFILL_CHEST,

		MORE_ITEM_Z_CHEST, MORE_ITEM_IMPROVED_CHEST

	}

	protected static ChestFillType rarefilltype;
	...
	public void makeFill(ChestFillType filltype) {
		...
		if(filltype == ChestFillType.MORE_ITEM_IMPROVED_CHEST || filltype == ChestFillType.MORE_ITEM_Z_CHEST) rarefilltype = filltype;
		if(rarefilltype == null) chests.entrySet().stream().forEach(k -> promptChestFill(k.getKey(), filltype));
		else chests.entrySet().stream().forEach(k -> promptChestFill(k.getKey(), rarefilltype));
		return;
	...
	public static class ChestBundler {
		...
		public static void commonMoreItemImproved(Chest chest, ChestType type) {
			...
			ChestPack pack = Chests.PACKAGER.getExtraPackage(MatchType.INSANE);
		...
		public static void commonMoreItemZ(Chest chest, ChestType type) {
			...
			ChestPack pack = Chests.PACKAGER.getExtraPackage(MatchType.Z);
			List<ItemStack> items = new ArrayList<ItemStack>();
