	...
	public static class TurretUtil {
		...
		public static class TurretBuilder extends BukkitRunnable {
			...
			public void setDefaults(TurretType turrettype, Block block) {
				...
				s1 = (ArmorStand) l.getWorld().spawnEntity(
						new Location(l.getWorld(), l.getX() + 0.5, l.getY() + 0.3, l.getZ() + 0.5),
						EntityType.ARMOR_STAND);
				s2 = (ArmorStand) l.getWorld().spawnEntity(
						new Location(l.getWorld(), l.getX() + 0.5, l.getY() - 0.2, l.getZ() + 0.5),
						EntityType.ARMOR_STAND);
			...
			public void run() {
				...
				Block b = l.getBlock();
				if (b.getType() == tt.getMaterial() && s1 != null && s2 != null && s1.isDead() == false
						&& s2.isDead() == false) {
					if (percent >= 100.00) {
						cancel();
						s1.remove();
		...
		public static void explodeTower(TurretType tt, Location l) {
			...
			for (int i = 0; i < 8; i++) {
				...
				ItemStack item = new ItemBuilder(explosion_mat).amount(1).durability(explosion_data)
						.name("" + NumberUtil.getRandomInt(-100, 100)).build();

				final Item itemdrop = l.getWorld().dropItemNaturally(l, item);
				itemdrop.setVelocity(new Vector(NumberUtil.getRandomDouble(0.1, 0.2),
						NumberUtil.getRandomDouble(0.1, 0.2), NumberUtil.getRandomDouble(0.1, 0.2)));
