...
public class FileFormatter extends Formatter {
	...
	public String format(LogRecord record) {
		...
		datePrefix = datePrefix.replace("%1%", ((hourOfDay < 10) ? "0" + hourOfDay : hourOfDay) + ":" + ((min < 10) ? "0" + min : min) + ":" + ((second < 10) ? "0" + second : second));
		...
		text +=  String.format("%s%n", datePrefix + className + ".java#" + record.getSourceMethodName() + "() : " +  record.getMessage());
