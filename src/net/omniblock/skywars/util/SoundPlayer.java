...
public class SoundPlayer {
	...
	public static void sendSound(Location l, String soundname){
		...
		for(Player cache : Bukkit.getOnlinePlayers()) {
			...
			PacketPlayOutNamedSoundEffect cache_h = new PacketPlayOutNamedSoundEffect(
					soundname, 
					cache.getLocation().getX(), 
					cache.getLocation().getY(), 
					cache.getLocation().getZ(), 
					5,
					1);
	...
	public static void sendSound(Player p, String soundname, boolean nearby, int radius){
		...
		if(nearby) {
			...
			PacketPlayOutNamedSoundEffect _p = new PacketPlayOutNamedSoundEffect(
					soundname, 
					l.getX(), 
					l.getY(), 
					l.getZ(), 
