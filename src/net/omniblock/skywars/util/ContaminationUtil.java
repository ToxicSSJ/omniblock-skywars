...
public class ContaminationUtil {
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					for(Location l : actived_troopers) {
						...
						if(l.getBlock().getType() == TROOPER_MATERIAL && l.getBlock().getData() == TROOPER_MATERIAL_DATA) {
											...
											for(BlockFace bf : bf_list) {
												if(l.getBlock().getRelative(bf).getType() == Material.AIR) {
													...
													for(int i = 0; i < NumberUtil.getRandomInt(1, 3); i++) {
														...
														l.getWorld().playSound(l, Sound.NOTE_BASS_DRUM, 5, NumberUtil.getRandomInt(-10, 10));
														l.getWorld().playSound(l, Sound.BLAZE_HIT, 5, NumberUtil.getRandomInt(-10, 10));
														
														Snowball bullet = (Snowball) l.getWorld().spawn(l.getBlock().getRelative(bf).getLocation().clone().add(0.5, 0, 0.5), Snowball.class);
														bullet.setMetadata("CONTAMINATION_BULLET", new FixedMetadataValue(Skywars.getInstance(), "dummy"));
	...
	public static class Contamination extends BukkitRunnable {
		...
		public boolean contamination(Block block) {
			...
			for(Block b : FACES) {
				if(b.getType() != Material.AIR && b.getType() != CONTAMINATION_MATERIAL && !actived_troopers.contains(b)) {
