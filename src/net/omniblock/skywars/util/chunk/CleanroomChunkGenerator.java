package net.omniblock.skywars.util.chunk;
...
public class CleanroomChunkGenerator extends ChunkGenerator {
	...
	public CleanroomChunkGenerator(String id) {
		if (id != null) {
			try {
					...
					for (int i = 0; i < tokens.length; i += 2) {
						...
						if (materialTokens.length == 2) {
							try {
								...
								log.warning("[CleanroomGenerator] Invalid Data Value '" + materialTokens[1]
										+ "'. Defaulting to 0.");
								dataValue = 0;
							}
						}
						...
						if (mat == null) {
							...
							if (mat == null) {
								log.warning("[CleanroomGenerator] Invalid Block ID '" + materialTokens[0]
										+ "'. Defaulting to stone.");
								mat = Material.STONE;
							}
						}
						...
						if (!mat.isBlock()) {
							log.warning("[CleanroomGenerator] Error, '" + materialTokens[0]
				...
				log.severe("[CleanroomGenerator] Error parsing CleanroomGenerator ID '" + id
	...
	public short[][] generateExtBlockSections(World world, Random random, int x, int z, BiomeGrid biomes) {
		...
		if (layer.length > maxHeight) {
			log.warning("[CleanroomGenerator] Error, chunk height " + layer.length
